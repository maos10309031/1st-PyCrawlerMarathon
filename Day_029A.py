...
class PttcrawlerSpider(scrapy.Spider):
    name = 'PTTCrawler'
    def __init__(self, board='Gossiping'): #define input data and change 'board' value 
        self.cookies = {'over18': '1'}
        self.host = 'https://www.ptt.cc'
        self.board = board
        self.start_urls = 'https://www.ptt.cc/bbs/{}/index.html'.format(board)
        super().__init__()   #繼承scrapy的參數功能 現在是空的

    def start_requests(self): #做出requests並加入parse中
        yield scrapy.Request(url=self.start_urls, callback=self.parse, cookies=self.cookies)

    def parse(self, response): #在parse中做文章版面搜索
        # 取得列表中的清單主體
        ...
        for div in main_list.findChildren('div', recursive=False):
            ...
            if class_name and 'r-ent' in class_name:
                ...
                yield scrapy.Request(url=article_URL,             #巢狀呼叫request加入清單
                                     callback=self.parse_article,
                                     cookies=self.cookies)

    def parse_article(self, response):  #在parse_article中做細部文章處理
        ...
        #
        # 透過 .stripped_strings 的方式可以快速移除多餘空白並取出文字(略過部分迭代值), 可參考官方文件 
        ...
        yield data #自動把檔案打包為generator的形式，不可直接調用，可以迭代方式調用
